...
namespace NLog.Targets
{
    ...
    public class RabbitMQ : TargetWithLayout
    {
        ...
        public RabbitMQ()
        {
            this.ConnectionKeyName = "RabbitMQServer";
            Layout = "${message}";
        ...
        private string GetTopic(LogEventInfo eventInfo)
        {
            ...
            routingKey = routingKey.Replace("{logger}", eventInfo.LoggerName).Replace("{level}", eventInfo.Level.Name);
            if (string.IsNullOrEmpty(routingKey))
            {
                routingKey = string.Format("{0}.{1}", eventInfo.Level.Name, eventInfo.LoggerName);
            }
            return routingKey;
        }
        ...
        {
            return _Encoding.GetBytes(MessageFormatter.GetMessageInner(_UseJSON, this.UseLayoutAsMessage, Layout, info.LogEvent, this.Fields, GetTopic(info.LogEvent)));
        }
        ...
        {
            InitConnectionParameterWithConnectionKeyName();
        ...
        /// <summary>
        /// 根据AppSetting中的ConnectionKeyName类配置RabbitMQ的连接字符串
        /// </summary>
        private void InitConnectionParameterWithConnectionKeyName()
        {
            if (!string.IsNullOrWhiteSpace(ConnectionKeyName))
            {
                var connString = ConfigurationManager.AppSettings[ConnectionKeyName];
                if (ConnectionStringBuilder.IsValidConnectionString(connString))
                {
                    ConnectionStringBuilder csb = new ConnectionStringBuilder(ConfigurationManager.AppSettings[ConnectionKeyName]);
